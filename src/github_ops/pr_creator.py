...
2. **Merge** this PR into your feature branch if tests look correct
3. **Request changes** by commenting `ai-revise: <your feedback>` on this PR
...
"""
        ...
        try:
            new_pr = repository.create_pull(
                title=f"AI Contract Tests for PR #{pr_number}",
                body=pr_body,
                head=test_branch,
                base=target_branch
            )
...
3. If you need changes, comment `ai-revise: <your feedback>` on the test PR
...
"""
    ...
    def post_skip_comment(
        ...
        comment = f"""## Contract Test Generation Skipped
...
If you believe this is incorrect, check that:
